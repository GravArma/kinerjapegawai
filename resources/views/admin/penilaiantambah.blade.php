                ...
                <div class="page-header">
                    <div class="row">
                        <div class="col-md-6 col-sm-12">
                            <div class="title">
                                <h4>Tambah Penilaian Kinerja Pegawai</h4>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="pd-20 card-box mb-30">
                    <form method="post" action="{{ url('admin/penilaiantambahsimpan') }}">
                        @csrf
                        <div class="form-group">
                            <label>Nama Pegawai</label>
                            <select name="idpegawai" class="form-control idpegawai selectcari" required>
                                <option value="">Pilih Pegawai</option>
                                <?php foreach ($pegawai as $row) { ?>
                                <option value="<?= $row->idpegawai ?>">
                                    <?= $row->nama ?></option>
                                <?php } ?>
                            </select>
                        </div>
